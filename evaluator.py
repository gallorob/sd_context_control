...
metrics = ['Image Quality', 'Image Visibility', 'Image Consistency',
           'Average Image Diversity (over runs)', 'Average Image Diversity (within run)',
...
brisque_eval = BRISQUE(url=False)
lpips_eval = LPIPS(net='alex', lpips=True)
...
working_dir = './image_context_test_results'
...
def compute_image_quality(image: PIL.Image) -> float:
	...
	return brisque_eval.score(image_arr)
...
def compute_image_diversity(image: PIL.Image,
                            ...
                            other_cropped_background: Optional[PIL.Image.Image] = None) -> float:
	image_with_background = cropped_background.copy()
	image_with_background.paste(image)
	...
	other_image_with_background.paste(other_image)

	image_with_background_tensor = th.from_numpy(np.array(image_with_background))
	image_with_background_tensor = th.reshape(image_with_background_tensor, (1, 3, image.size[1], image.size[0]))
	other_image_with_background_tensor = th.from_numpy(np.array(other_image_with_background))
	other_image_with_background_tensor = th.reshape(other_image_with_background_tensor,
	                                                (1, 3, image.size[1], image.size[0]))

	return lpips_eval.forward(image_with_background_tensor, other_image_with_background_tensor).detach().cpu().numpy()[
		0]
...
for k, exp_name in enumerate(experiments):
	for i, (room_name, room_description) in enumerate(rooms.items()):
		for j, (entity_name, entity_description) in enumerate(enemies.items()):
			...
			room_image_cropped = PIL.Image.open(f'{working_dir}/{exp_name}/{room_name}_cropped.png')
			room_image_cropped = room_image_cropped.resize(entity_image_size)
			for context_level in context_levels:
				entity_image = PIL.Image.open(
					f'{working_dir}/{exp_name}/{entity_name.lower()}_{room_name}_{context_level}.png')
				...
				for other_context_level in context_levels:
					if context_level != other_context_level:
						other_image = PIL.Image.open(
							f'{working_dir}/{exp_name}/{entity_name.lower()}_{room_name}_{other_context_level}.png')
				...
				for other_exp_name in experiments:
					if exp_name != other_exp_name:
						other_image = PIL.Image.open(
							f'{working_dir}/{other_exp_name}/{entity_name.lower()}_{room_name}_{context_level}.png')
				...
				for other_room_name, _ in rooms.items():
					if room_name != other_room_name:
						...
						image_diversities_across_rooms.append(
							compute_image_diversity(entity_image, entity_image, room_image_cropped,
							                        other_room_image_cropped))
				results_data.loc[len(results_data)] = [k, room_name, entity_name, context_level, image_quality,
				                                       image_visibility, image_consistency,
...
# plt.tight_layout()
# plt.savefig(f'{working_dir}/{metric}_alt.png', transparent=True)
...
for metric in metrics:
	...
	for context_level in context_levels:
		n = 0
		v0 = \
		processed_data.loc[(processed_data['Metric'] == metric) & (processed_data['Context Level'] == context_level)][
			'Mean'].item()
